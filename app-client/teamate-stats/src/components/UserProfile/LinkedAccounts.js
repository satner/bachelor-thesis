  ...
  <Mutation mutation={DELETE_SUMMONER}>
    {(deleteSummoner, { data }) => (
      <Button
        shape="circle"
        icon="delete"
        value={props.id + "+" + props.item.name + "+" + props.item.server}
        onClick={e => {
            ...
            .then(res => {
              if (res.data.deleteSummoner) {
                openNotificationWithIcon(
                  "success",
                  "Success",
                  "Summoner Account Deleted!"
                );
              } else {
                openNotificationWithIcon(
                  "warning",
                  "Error",
                  "Summoner Account has not Deleted!"
                );
              }
            })
            .catch(rej => {
      ...
      return (
        <List
          ...
          renderItem={item => (
            <List.Item
              actions={[
                <DeleteSummoner item={item} id={data.getUserInfos._id} />
              ]}
            >
              <List.Item.Meta
        ...
        {(addSummoner, { data }) => (
          <div>
            <SummonerAccounts
              ...
              serverNameFunc={this.unfoldServerName}
            />
            <Modal
                ...
                >
                  {getFieldDecorator("summoner", {
                    rules: [
                      {
                        required: true,
                        message: "Please input your summoner name!",
                        whitespace: true
                      }
                    ]
                  })(<Input />)}
                ...
                >
                  {getFieldDecorator("server", {
                    rules: [
                      {
                        required: true,
                        message: "Please select server game!",
                        whitespace: true
                      }
                    ]
                  })(
